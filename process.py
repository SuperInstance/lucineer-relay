#!/usr/bin/env python3
"""
Lucineer Fast Job Processor — Python Edition
Polls the Worker for pending jobs and processes them with build templates.
No model calls needed — pure pattern matching with Lucineer's voice.
"""
import json, sys, os, time, urllib.request, urllib.error, random
...
def api_get(path):
    import subprocess
    result = subprocess.run(['curl', '-s', '-H', f'X-Lucineer-Key: {AUTH_KEY}',
                           f'{WORKER_URL}{path}'], capture_output=True, text=True, timeout=10)
...
def b_garden(px, py, pz):
    ...
    return ("Bed's in, fence is up, six flowers and a tree at center. "
...
KEYWORDS = {
    ...
    'dome': b_dome, 'observatory': b_dome, 'dome': b_dome, 'bubble': b_dome,
...
def process_job(job):
    ...
    try:
        ...
        log(f"  ✓ Complete ({len(commands)} commands)")
        return True
...
def run_loop(interval=2):
    log("=== Lucineer Python Processor — continuous mode ===")
    while True:
        try:
            run_once()
        except Exception as e:
...
if __name__ == "__main__":
    ...
    if mode == "--loop":
        ...
        mock = {"id":"mock_test","playerName":"Casey","message":"build me a castle",
